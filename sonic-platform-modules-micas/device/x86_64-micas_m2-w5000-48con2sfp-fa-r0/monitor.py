    ...
    def getETValue(a, filename, tagname):
        ...
        for neighbor in root.iter(tagname):
            ...
            for pros in neighbor.iter("property"): 
                ...
                prob_t['errcode']= 0
                if ('type' not in list(ret.keys())):
                    val = "0";
                else:
                    val = ret["type"]
                if ('bit' not in list(ret.keys())):
                    bit = "0"; 
                else:
                    bit = ret["bit"]
                ...
                if  isinstance(s, str) and s.startswith("ERR"):
                    prob_t['errcode']= -1
                    prob_t['errmsg']= s
                if ('default' in list(ret.keys())):  # Need to check
                    rt = status.getDecodValue(root,ret['decode'])# get a code value of 7
                    ...
                    if ('decode' in list(ret.keys())):
                        rt = status.getDecodValue(root,ret['decode'])# get a code value of 7
                        s = rt[s.replace("\x00","").rstrip()]
                name = ret["name"]
                prob_t[name]=str(s)
            a.append(prob_t)
    @staticmethod
